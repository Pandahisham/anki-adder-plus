...
export class Marking {
    ...
    splitText(id, searchForClozeMarkup, text) {
        //2 arguments: Splits the span 'id'
        if (arguments.length == 2) {
            this.splitText(id, searchForClozeMarkup, id.innerHTML.replace(/&nbsp;/g, ' ').replace(/(\r|\t|\v|\f)/g, " ")
            ...
            if (searchForClozeMarkup) {
                ...
                if (r) {
                    ...
                    this.splitText(id, false, txt1);
                    insertText(id, r[2].replace(/ /g, "&nbsp;"), Number(r[1]));
                    this.splitText(id, true, txt2);
                    $(id).remove();
                    return;
                }
            }
    ...
    isP(character) { //Returns whether a character is a punctuation
        return /[.!?:,;_'"”“´`()\[\]{}<>*+|$%&/\\\-]/.test(character);
    }

    isEarlier(idA, idB) { //Returns whether idA comes before idB
        return (idB.compareDocumentPosition(idA) & Node.DOCUMENT_POSITION_PRECEDING);
    }

    getWPSN(character) { //Returns which type a single character is, excluding the cloze type.
        if (character == ' ')
            return 's';
        if (character == '\n')
            return 'n';
    ...
    getLowestC(generateLastC) { //Returns the lowest non-used c-value
        for (var i = 1; i <= 10; i++) {
    ...
    getClozeText(normalize, mouseX, mouseY) {
    ...
    clozeField() {
            ...
            .on({
                'focus': function () {
                    ...
                    if (!$(".w:hover, .p:hover, .s:hover, .c:hover").length) {
                        this.clozeArea(e.pageX, e.pageY);
                        setTimeout(function () {
                            $("#clozearea").focus()
                        }, 1);
                    }
        ...
        this.splitText(id, true);
        markingC = getLowestC(true);
        updateLayout();
        updateScrollWidth();
    }

    htmlToClozeText(s) { //Same as in eventPage.ts
        changedWhileCloze = true;
        if (s === undefined || s == "<br>")
            return "";
        else

...
export class AnkiWeb {
    ...
    currentXhr = undefined; //Is null when finished

    connectToAnki(successCallback, errorCallback, forceRelogin) {
        ...
        if (forceRelogin || !localStorage["login-status"] || localStorage["login-status"] != "OK") {
            ...
            this.currentXhr = $.get('https://ankiweb.net/account/logout', function (data, textStatus) { //Start with logging any other user off.
                console.log("Login to AnkiWeb");
                this.currentXhr = $.post('https://ankiweb.net/account/login', { //Submit user info
                        submitted: "1",
    ...
    retrieveData(successCallback) {
        ...
        this.currentXhr = $.get('https://ankiweb.net/edit/', function (data, textStatus) {
    ...
    addNote(dontClose) {
        ...
        if (this.currentXhr && this.currentXhr.readyState != 4) { //If the connection to AnkiWeb is not completed, restart it and add note when it's finished
        ...
        this.currentXhr = $.get('https://ankiweb.net/edit/save',
            dict,
            function (data, textStatus) {
                if (textStatus == 'error') {
    ...
    updateContextMenu() { //Same as in eventPage.ts
